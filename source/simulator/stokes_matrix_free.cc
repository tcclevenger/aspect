...
//  }
  ...
  std::pair<double,double> StokesMatrixFreeHandler<dim>::krylov_solve()
  {
    ...
    // in such a way to be a solver
    MGCoarseGridApplySmoother<vector_t> mg_coarse;
    mg_coarse.initialize(mg_smoother_A);
    ...
    MassPreconditioner prec_S(dof_handler_p, mg_mass, mg_transfer_mass);
...
//  PrimitiveVectorMemory<dealii::LinearAlgebra::distributed::BlockVector<double> > mem_idr2;
    ...
    if ((sim.parameters.n_cheap_stokes_solver_steps == 0
         && sim.parameters.n_expensive_stokes_solver_steps == 0)
        ||
        (sim.parameters.n_cheap_stokes_solver_steps > 0
         && sim.parameters.n_expensive_stokes_solver_steps > 0))
      {
        AssertThrow(false,ExcMessage("Choose either cheap OR expensive Stokes"));
      }
        ...
        try
          {
            ...
            timer.stop();
            const double solve_time = timer.last_wall_time();
            fgmres_m = actual_solver_control_expensive.last_step();
            sim.pcout << "   FGMRES Solved in " << fgmres_m << " expensive iterations (" << solve_time << " : "
                      << preconditioner_expensive.n_iterations_A()/(1.0*fgmres_m)<< " : "
                      << preconditioner_expensive.n_iterations_S()/(1.0*fgmres_m) << ")."
                      << std::endl;
        ...
        try
          {
            ...
            timer.stop();
            const double solve_time = timer.last_wall_time();
            idr1_m = actual_solver_control_expensive.last_step();
            sim.pcout << "   IDR(1) Solved in " << idr1_m << " expensive iterations (" << solve_time << " : "
                      << preconditioner_expensive.n_iterations_A()/(2.0*idr1_m) << " : "
                      << preconditioner_expensive.n_iterations_S()/(2.0*idr1_m) << ")."
                      << std::endl;
        ...
        try
          {
            ...
            timer.stop();
            const double solve_time = timer.last_wall_time();
            idr2_m = actual_solver_control_expensive.last_step();
            sim.pcout << "   IDR(2) Solved in " << idr2_m << " expensive iterations (" << solve_time << " : "
  ...
  void StokesMatrixFreeHandler<dim>::setup_dofs()
  {
    ...
    mg_constrained_dofs_p.clear();
    mg_constrained_dofs_p.initialize(dof_handler_p);
      ...
      for (unsigned int level=0; level<n_levels; ++level)
        {
          ...
          level_constraints.close();
          {
            ...
            mg_matrices_mass[level].initialize(mg_mf_storage_level, mg_constrained_dofs_p, level);
    ...
    mg_transfer_mass.initialize_constraints(mg_constrained_dofs_p);
