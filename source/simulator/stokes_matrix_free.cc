...
namespace aspect
{
  ...
  std::pair<double,double> StokesMatrixFreeHandlerImplementation<dim,velocity_degree>::solve()
  {
    ...
    dealii::LinearAlgebra::distributed::BlockVector<double> solution_copy3(2);

    stokes_matrix.initialize_dof_vector(solution_copy2);
    stokes_matrix.initialize_dof_vector(solution_copy3);
    ...
    sim.pcout << std::endl << "InitialGuess: " << solution_copy.l2_norm()
              << std::endl << "SolverTOL:    " << solver_tolerance
